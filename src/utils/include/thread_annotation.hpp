...
#define DUCKDB_REQUIRES(...) \
  THREAD_ANNOTATION_ATTRIBUTE__(requires_capability(__VA_ARGS__))
#define DUCKDB_REQUIRES_SHARED(...) \
...
#define DUCKDB_ACQUIRE(...) \
  THREAD_ANNOTATION_ATTRIBUTE__(acquire_capability(__VA_ARGS__))
#define DUCKDB_ACQUIRE_SHARED(...) \
...
#define DUCKDB_ACQUIRED_BEFORE(...) \
  THREAD_ANNOTATION_ATTRIBUTE__(acquired_before(__VA_ARGS__))
#define DUCKDB_ACQUIRED_AFTER(...) \
...
#define DUCKDB_RELEASE(...) \
  THREAD_ANNOTATION_ATTRIBUTE__(release_capability(__VA_ARGS__))
#define DUCKDB_RELEASE_SHARED(...) \
  THREAD_ANNOTATION_ATTRIBUTE__(release_shared_capability(__VA_ARGS__))
#define DUCKDB_RELEASE_GENERIC(...) \
...
#define DUCKDB_RETURN_CAPABILITY(x) \
...
// DUCKDB_ACQUIRE.
#define DUCKDB_TRY_ACQUIRE(...) \
  THREAD_ANNOTATION_ATTRIBUTE__(try_acquire_capability(__VA_ARGS__))
#define DUCKDB_TRY_ACQUIRE_SHARED(...) \
...
// annotated function.
#define DUCKDB_ASSERT_CAPABILITY(x) \
  THREAD_ANNOTATION_ATTRIBUTE__(assert_capability(x))
#define DUCKDB_ASSERT_SHARED_CAPABILITY(x) \
