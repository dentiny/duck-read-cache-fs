...
template <typename M>
class DUCKDB_SCOPED_CAPABILITY lock_guard
  : public internal::lock_impl_t<lock_guard<M>> {
 private:
...
template <typename M>
class DUCKDB_SCOPED_CAPABILITY unique_lock
  : public internal::lock_impl_t<unique_lock<M>> {
 private:
 ...
 public:
  ...
  unique_lock(M& m, std::defer_lock_t t) noexcept DUCKDB_EXCLUDES(m)
    : Impl(m, t) {}
